...
export function ChartLineSelector({ chartLines, modelNames, maxLines = 9, onChange }: ChartLineSelectorProps) {
        ...
        <Card
            ...
            extra={
                <div className={styles.chartLineHeader}>
                    ...
                    <Button variant="secondary" size="sm" onClick={handleAdd} disabled={chartLines.length >= maxLines}>
            ...
            <div className={styles.chartLineList}>
                {chartLines.map((line, index) => (
                    <div key={index} className={styles.chartLineItem}>
                        <span className={styles.chartLineLabel}>{t(`usage_stats.chart_line_label_${index + 1}`)}</span>
                        <select
                            value={line}
