...
export function RequestLogs({ data, loading: parentLoading, providerMap, providerModels, providerTypeMap, sourceInfoMap, authFileMap: propAuthFileMap, apiFilter }: RequestLogsProps) {
  ...
  const renderRow = (entry: LogEntry) => {
    ...
    const authDisplayName = authFileMap.get(entry.authIndex)?.name || entry.authIndex || '-';
