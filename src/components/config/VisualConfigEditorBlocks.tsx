...
function getRuleCardClassName(baseClassName: string, disabled?: boolean) {
...
const StringListEditor = memo(function StringListEditor({
  ...
  value: PayloadFilterParamEntry[];
  disabled?: boolean;
  placeholder?: string;
  inputAriaLabel?: string;
  onChange: (next: PayloadFilterParamEntry[]) => void;
}) {
  const { t } = useTranslation();
  ...
  const updateItem = (index: number, patch: Partial<PayloadFilterParamEntry>) =>
    onChange(items.map((item, i) => (i === index ? { ...item, ...patch } : item)));
  const addItem = () => {
    setItemIds([...renderItemIds, makeClientId()]);
    onChange([...items, { id: makeClientId(), path: '', disabled: false }]);
  };
  ...
  const toggleItemDisabled = (index: number) => {
    ...
    if (!item) return;
    updateItem(index, { disabled: !item.disabled });
  };
  ...
  return (
    <div className={styles.stringList}>
      {items.map((item, index) => {
        ...
        return (
          <div
            key={renderItemIds[index] ?? item.id ?? `item-${index}`}
            className={[
              styles.stringListRow,
              itemDisabled ? styles.stringListRowDisabled : '',
            ]
              .filter(Boolean)
              .join(' ')}
          >
            ...
            <ExpandableInput
              ...
              value={item.path}
              onChange={(nextValue) => updateItem(index, { path: nextValue })}
              disabled={disabled || itemDisabled}
            />
            <Button
              variant={itemDisabled ? 'secondary' : 'ghost'}
              size="sm"
              onClick={() => toggleItemDisabled(index)}
              disabled={disabled}
            >
              {itemDisabled
