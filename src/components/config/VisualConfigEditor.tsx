...
export function VisualConfigEditor({
  ...
  const [rotatingTor, setRotatingTor] = useState(false);
  const [showTorPassword, setShowTorPassword] = useState(false);
  const [showSecretKey, setShowSecretKey] = useState(false);
  const [proxyTestResult, setProxyTestResult] = useState<{ success?: boolean; ip?: string; country?: string; error?: string } | null>(null);
  const [torRotateResult, setTorRotateResult] = useState<{ success?: boolean; message?: string; old_ip?: string; new_ip?: string; error?: string } | null>(null);
  ...
  const handleTestProxy = useCallback(async () => {
    ...
    try {
      ...
      setProxyTestResult({ success: false, error: msg });
    } finally {
      setTestingProxy(false);
    }
  }, []);

  const handleTorRotate = useCallback(async () => {
    setRotatingTor(true);
    setTorRotateResult(null);
    try {
      const data = await apiClient.post<{ success?: boolean; message?: string; new_ip?: string; error?: string }>('/tor-rotate');
      setTorRotateResult(data);
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : 'Request failed';
      setTorRotateResult({ success: false, error: msg });
    } finally {
      setRotatingTor(false);
    }
  }, []);
  ...
  const proxyUrlField = (
    <FieldAnchor fieldId="proxyUrl">
      <Input
        ...
        disabled={disabled || isTorMode}
      />
    </FieldAnchor>
  );

  const proxyModeField = (
      ...
      >
        <Select
          ...
          options={[
            {
              value: 'proxy',
              label: t('config_management.visual.sections.network.proxy_mode_proxy'),
            },
            {
              value: 'tor',
              label: t('config_management.visual.sections.network.proxy_mode_tor'),
            },
          ]}
          ...
          onChange={(nextValue) => {
            ...
            if (proxyMode === 'tor') {
              ...
              patch.torRetryOnCodesText = values.torRetryOnCodesText || '429, 403, 500, 502, 503';
            }
            onChange(patch);
          }}
        />
                      ...
                      <FieldAnchor fieldId="rmSecretKey">
                        <Input
                          ...
                          rightElement={
                            ...
                            >
                              {showSecretKey ? <IconEyeOff size={18} /> : <IconEye size={18} />}
                            </button>
                          }
                        />
                      </FieldAnchor>
                ...
                <SectionGrid>
                  ...
                  {isTorMode ? null : (
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
                      ...
                      <div style={{ paddingTop: '24px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                        <Button
                          variant="secondary"
                          size="sm"
                          loading={testingProxy}
                          disabled={disabled || testingProxy}
                          onClick={handleTestProxy}
                        >
                          Check IP
                        </Button>
                        {proxyTestResult && (
                          <div style={{ fontSize: '12px', lineHeight: '1.3', color: proxyTestResult.success ? 'var(--color-success)' : 'var(--color-error)' }}>
                            {proxyTestResult.success ? (
                              <>{proxyTestResult.ip}{proxyTestResult.country ? ` (${proxyTestResult.country})` : ''}</>
                            ) : (
                              <>{proxyTestResult.error || 'Failed'}</>
                            )}
                          </div>
                        )}
                      </div>
                    ...
                    <>
                      ...
                      <FieldAnchor fieldId="torControlPassword">
                        <Input
                          ...
                          type={showTorPassword ? 'text' : 'password'}
                          ...
                          rightElement={
                            <button
                              type="button"
                              onClick={() => setShowTorPassword(!showTorPassword)}
                              style={{
                                background: 'none',
                                border: 'none',
                                cursor: 'pointer',
                              ...
                              aria-label={showTorPassword ? 'Hide password' : 'Show password'}
                            >
                              {showTorPassword ? <IconEyeOff size={18} /> : <IconEye size={18} />}
                            </button>
                          }
                        />
                      </FieldAnchor>
                      ...
                      <FieldAnchor fieldId="torRotateButton">
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                          ...
                          <Button
                            ...
                            size="sm"
                            loading={rotatingTor}
                            disabled={disabled || rotatingTor}
                            onClick={handleTorRotate}
                          >
                            Rotate Tor IP (NEWNYM)
                          </Button>
                          {torRotateResult && (
                            <div style={{ fontSize: '12px', lineHeight: '1.3', color: torRotateResult.success ? 'var(--color-success)' : 'var(--color-error)' }}>
                              {torRotateResult.success ? (
                                <>{torRotateResult.message}
                                  {torRotateResult.old_ip ? ` — Old IP: ${torRotateResult.old_ip}` : ''}
                                  {torRotateResult.new_ip ? ` → New IP: ${torRotateResult.new_ip}` : ''}
                                </>
                              ) : (
                                <>{torRotateResult.error || 'Failed'}</>
                              )}
                            </div>
                          )}
                        </div>
