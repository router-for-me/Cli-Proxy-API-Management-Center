...
import { IconBot, IconCode, IconChevronUp, IconDownload, IconInfo, IconTrash2 } from '@/components/ui/icons';
...
const resolveQuotaErrorMessage = (t: TFunction, status: number | undefined, fallback: string): string => {
...
export function AuthFilesPage() {
    ...
    const quotaFilterType: QuotaProviderType | null = QUOTA_PROVIDER_TYPES.has(normalizedFilter as QuotaProviderType)
    ...
    }, [prefixProxyEditor?.json, prefixProxyEditor?.prefix, prefixProxyEditor?.proxyUrl, prefixProxyEditor?.rawText]);
    ...
    const handleMappingUpdate = async (provider: string, sourceModel: string, newAlias: string) => {
        ...
        if (
            currentMappings.some(
                (m) =>
                    (m.name ?? '').trim().toLowerCase() === nameKey && (m.alias ?? '').trim().toLowerCase() === aliasKey
            )
        ) {
            return;
        }
    ...
    const handleToggleFork = async (provider: string, sourceModel: string, alias: string, fork: boolean) => {
        const normalizedProvider = normalizeProviderKey(provider);
        if (!normalizedProvider) return;
    ...
    const renderFilterTags = () => (
        <div className={styles.filterTags}>
            {existingTypes.map((type) => {
                ...
                const color =
                    type === 'all' ? { bg: 'var(--bg-tertiary)', text: 'var(--text-primary)' } : getTypeColor(type);
    ...
    const updateQuotaState = useCallback(
        (type: QuotaProviderType, updater: (prev: Record<string, unknown>) => Record<string, unknown>) => {
            if (type === 'antigravity') {
